 ...
 */
const logSupabaseCall = (operation, details) => {
...
export const withTimeout = async (promise, timeoutMs = 30000) => {
    return Promise.race([
        promise,
        new Promise((_, reject) => setTimeout(() => reject(new Error(`API request timed out after ${timeoutMs}ms`)), timeoutMs))
    ]);
};
/**
 ...
 */
export const withRetry = async (fn, maxRetries = 3, initialDelayMs = 500) => {
    let lastError;
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`⚠️ Attempt ${attempt}/${maxRetries} failed, retrying in ${delayMs}ms:`, error.message);
 ...
 */
export const createInterview = async (authUserId, settings) => {
    try {
...
// --- Data Fetching ---
export const getInterviewsForUser = async (authUserId) => {
...
export const getReportForInterview = async (interviewId) => {
    const { data, error } = await supabase
        .from('performance_reports')
        .select('*')
...
export const getCommentsForInterview = async (interviewId) => {
    const { data, error } = await supabase
        .from('comments')
...
export const getRecordingDownloadUrl = async (mediaPath) => {
...
export const getAnswersForInterview = async (questionIds) => {
 ...
 */
export const createAuditLog = async (userId, action, entityId, tableName, details) => {
    try {
 ...
 */
export const scheduleInterview = async (authUserId, settings) => {
    try {
        // Get user's internal ID
        const { data: userProfile, error: profileError } = await supabase
            .from('users')
        ...
        if (interviewError)
            throw interviewError;
        await createAuditLog(authUserId, 'INTERVIEW_SCHEDULE', newInterview.id, 'interviews', {
            changes: {
                created: {
 ...
 */
export const getScheduledInterviews = async (userId) => {
    try {
...
export const cancelInterview = async (interviewId, userId) => {
    try {
        ...
        if (success) {
            await createAuditLog(userId, 'INTERVIEW_CANCEL', interviewId, 'interviews', {
                changes: {
                    updated: {
                        status: 'cancelled'
                    }
...
export const deleteInterview = async (interviewId, userId) => {
    try {
        ...
        const success = await updateInterviewStatus(interviewId, 'cancelled');
        if (success) {
            await createAuditLog(userId, 'DATA_DELETE', interviewId, 'interviews', {
                changes: {
                    updated: {
                        status: 'cancelled',
                        deleted: true
